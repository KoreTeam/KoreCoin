...
repositories. However it's in the korecoin repository, which we can add
with:

    apt-add-repository ppa:korecoin/korecoin
...
.. _`dependencies archive`: https://download.visucore.com/korecoin/qtgui_deps_1.zip
.. [#] PGP signature: https://download.visucore.com/korecoin/qtgui_deps_1.zip.sig (signed with RSA key ID `610945D0`_)
...
The static binary version of KoreCoin is linked against libdb4.8 (see also `this Debian issue`_).
...
installed causes korecoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
