...
CBigNum bnProofOfStakeLimitTestNet(~uint256(0) >> 30);

unsigned int nTargetSpacing = 60; // 1.5 minutes
unsigned int nStakeTargetSpacing = 60; // 3.5 minutes stakes spacing
unsigned int nStakeMinAge = 60 * 60 * 1; // 1 days as zero time weight
unsigned int nStakeMaxAge = 60 * 60 * 8; // 6 month as full weight
...
static const int64_t nTargetSpacingWorkMax = 3 * nTargetSpacing; 
...
const string strMessageMagic = "KoreCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees, unsigned int nBits)
{
    ...
    if(nHeight > CUTOFF_POW_STN_BLOCK) {
        nSubsidy = 0;
	return nSubsidy;
    }
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, unsigned int nBits, unsigned int nTime, bool bCoinYearOnly)
{
    int64 nRewardCoinYear;
    nRewardCoinYear = MAX_PROOF_OF_STAKE;
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if (pindexLast->nHeight >= 22150)
        return bnTargetLimit.GetCompact(); // Save The Net
...
inline bool CheckProofOfWorkSilent(uint256 hash, unsigned int nBits)
{
    CBigNum bnTarget;
    bnTarget.SetCompact(nBits);
    return bnTarget > 0 && bnTarget <= bnProofOfWorkLimit && hash <= bnTarget.getuint256();
}
...
{
    nTime = max((int64_t)GetBlockTime(), (int64_t)GetAdjustedTime());
}
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            {
                // Verify signature
                if (!VerifySignature(txPrev, *this, i, 0) && nBestHeight > LASTFORKBLOCK)
                {
        ...
        if (!IsCoinStake())
        {
            ...
            if (nTxFee < GetMinFee() && nBestHeight > LASTFORKBLOCK)
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfWork())
    {
        int64_t nReward = GetProofOfWorkReward(pindex->nHeight, nFees, nBits);
    ...
    if (IsProofOfStake())
    {
        ...
        if (nStakeReward > nCalculatedStakeReward && pindex->nHeight > LASTFORKBLOCK)
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProofOfWork() && nHeight > CUTOFF_POW_STN_BLOCK)
    ...
    if (IsProofOfStake() && nHeight > LASTFORKBLOCK)
    {
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (pcheckpoint && pblock->hashPrevBlock != hashBestChain && !Checkpoints::WantedByPendingSyncCheckpoint(hash))
    {
        ...
        if (bnNewBlock > bnRequired && pindexBest->nHeight > LASTFORKBLOCK)
        {
            if (pfrom)
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "KoreCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "CoinDesk 14/06/14 - Blockchain Launches Bitcoin.com";
        CTransaction txNew;
        txNew.nTime = 1402729860;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1402729860;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2758747;
		

        //// debug print
	block.print();
        assert(block.hashMerkleRoot == uint256("0x5cec68dbbb3874c7465a4c222065d900b34955aa2503460094e257993f0ae177"));   
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
	...
	printf("receive version message: version %d, blocks=%d, us=%s, them=%s, peer=%s, verification=%" PRId64 "\n", pfrom->nVersion, pfrom->nStartingHeight, addrMe.ToString().c_str(), addrFrom.ToString().c_str(), pfrom->addr.ToString().c_str(), verification_token);
