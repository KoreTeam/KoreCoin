...
public:
    ...
    bool GetReconnectToken(const CNetAddr &addr, uint64_t& reconnect_token)
    {
        bool result = false;
        {
            LOCK(cs);
            Check();
            std::map<CNetAddr, uint64_t>::const_iterator found = reconnectToken.find(addr);
    ...
    bool CheckVerificationToken(const CNetAddr &addr, uint64_t verification_token)
    {
        bool result = false;
        {
            LOCK(cs);
            Check();
            std::map<CNetAddr, uint64_t>::const_iterator found = verificationToken.find(addr);
            if (verificationToken.end() != found) {
