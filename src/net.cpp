...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        string strDesc = "KoreCoin " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
static const char *strMainNetOnionSeed[][1] = {
    {"ta56fvoyjmw3wdss.onion"},
    {"3bs3bk4opaasrgc6.onion"},
    {"hxjnydhtwd74c3ke.onion"},
    {NULL}
};

void ThreadOnionSeed(void* parg)
{
    // Make this thread recognisable as the Onion seeding thread
    RenameThread("korecoin-onionseed");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. KoreCoin is probably already running."), addrBind.ToString().c_str());
        else
